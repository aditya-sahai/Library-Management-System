...
def open_book_status_menu():
    ...
    def statusbutton():
        ...
        result = DataManager.book_status(fbookisbn)
        if result == "in-shelf":
            messagebox.showinfo("Available", "Book is available for issuing")
            book_name_entry.delete(0, END)
            phone_no_entry.delete(0, END)
            book_isbn_entry.delete(0, END)
        elif result == "not-in-shelf":
            messagebox.showinfo("Unavailable", "Book is unavailable. To check for copies, try entering \"-01\",\"-02\" and so on.")
            book_name_entry.delete(0, END)
            phone_no_entry.delete(0, END)
            book_isbn_entry.delete(0, END)
        elif result == "isbn":
