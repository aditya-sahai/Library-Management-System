...
def open_return_book_menu():
    ...
    def returnbook():
        ...
        try:
            fphoneno = int(fphoneno)
        except:
            response = messagebox.showerror("Error", "Phone no. can only have numbers!")
        try:
            fbookisbn = int(fbookisbn)
        except:
            response = messagebox.showerror("Error", "ISBN can only have numbers!")

        if len(str(fphoneno))!=10:
